"""session pedagogical fields

Revision ID: 34f5a6b7c8d9
Revises: 33e4f5a6b7c8
Create Date: 2026-04-30

Ajoute les champs pédagogiques portés par la séance :
- intention de séance ;
...
revision = "34f5a6b7c8d9"
down_revision = "33e4f5a6b7c8"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    with op.batch_alter_table("session_activite") as batch:
        for name, col in cols:
            if not _has_column("session_activite", name):
                batch.add_column(col)

    # Récupération douce des intentions créées provisoirement comme Objectif.session_id.
    # On copie la première intention trouvée, sans supprimer les objectifs existants.
    if _has_table("objectif") and _has_column("session_activite", "intention_seance"):
        bind = op.get_bind()
