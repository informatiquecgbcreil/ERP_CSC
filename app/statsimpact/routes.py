...
from app.services.consumption import aggregate_sessions_consumption, calculate_session_consumption
...
def _build_statsimpact_scope_workbook(flt) -> Workbook:
    ...
    _append_section(ws_at, 'Vue ateliers', ['Secteur','Atelier','Type','Sessions prév.','Sessions réelles','Présences','Uniques','Heures réalisées',"Taux d'occupation (%)",'Moyenne / séance','Durée activité (j)','Conso kWh','CO₂ kg','Séances équipées'], atelier_rows, start_row=1)
...
def _build_magato_per_atelier_workbook(flt) -> Workbook:
    ...
    for _gid, members in grouped_ateliers.items():
        ...
        if not pres_rows or not pid_set:
            ...
            ws.freeze_panes = f"F{matrix_header_row + 1}"
